...
1. **`docs/report/` 보고서 우선**: 현재 브랜치 이슈 번호 또는 날짜로 매칭되는 보고서가 있으면 그걸 1차 소스로 사용
   - **보고서가 없으면 먼저 생성한다** — `/report` 규약(`.claude/commands/report.md`)대로 `docs/report/[YYYYMMDD]_#[이슈번호]_[설명].md`를 만든 뒤 이 커맨드를 이어간다. 보고서 없이 PR 본문만 만들지 않는다
...
리뷰어가 스크롤하기 전에 "무엇이 왜 바뀌었는지"가 보여야 한다.

- **결론 먼저.** Summary 두 문장 안에서 끝낸다. 경위·근거는 그 아래로
- **셋 이상은 표로.** 변경 파일·Before/After·환경별 동작은 불릿 대신 표
- **강조는 GitHub 알림으로** — `> [!WARNING]` 놓치면 사고 나는 것 / `> [!IMPORTANT]` 머지 전에 확인이 필요한 조건 / `> [!NOTE]` 알아두면 좋은 배경
- **긴 근거는 접어서.** 전체 diff·로그·마이그레이션 SQL은 `<details><summary>`
- **`<sub>`·`<small>` 금지.** 글씨가 너무 작아 아무도 안 읽는다. 중요하면 본문 크기로 쓰고, 안 중요하면 지운다
- **위치를 짚어서.** `google_map_view.dart:42`처럼 파일·라인·이슈 번호로 쓴다
...
## PR 등록 흐름 (사용자가 PR 생성까지 요청한 경우)

본문 출력에서 끝내지 않고 PR 등록까지 요청받았으면, 순서를 지킨다 — **리포트 → 이슈 댓글 → PR**.

1. **리포트 확보**: `docs/report/`에서 브랜치 이슈 번호로 매칭. 없으면 `/report` 규약으로 먼저 생성
2. **이슈 댓글**: 리포트 원문을 해당 이슈에 그대로 단다
   ```bash
   gh issue comment <이슈번호> --body-file "docs/report/<리포트 파일>.md"
   ```
   - 같은 리포트가 이미 댓글로 달려 있으면 생략 (중복 댓글 금지)
3. **PR 생성**: 위에서 만든 본문으로
   ```bash
   gh pr create --base <대상 브랜치> --title "<type> : <한글 설명> #N" --body-file <본문 파일>
   ```
   - base는 사용자가 지정한 브랜치 (에픽 진행 중엔 보통 에픽 브랜치, 예: `20260812_#470_...`)
   - push는 하지 않는다 — 원격에 브랜치가 없으면 사용자에게 push를 요청한 뒤 진행
...
- 사용자가 명시적으로 저장을 원할 때만 `docs/pr/` 디렉토리에 저장
  - 경로: `docs/pr/{브랜치명}.md`
  - `docs/pr/`가 `.gitignore`에 없으면 추가 안내
