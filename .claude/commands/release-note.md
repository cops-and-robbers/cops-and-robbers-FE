...
- 톤: **간결하고 담백하게**. 사실만
- 두 섹션 구성: 상단 요약 + 하단 한국어/일본어/영어 상세 (앱이 ko/ja/en 3개 로케일 지원)
...
**섹션 2 — 하단 상세 (한국어 + 일본어 + 영어)**
- 한국어 → 일본어 → 영어 순서, 각 구간 사이에 구분선(`==========`)
- 각 기능을 번호 항목으로, 세부 내용은 `-` 불릿으로
- 기존 변경 전/후 문구가 있으면 "기존/변경" 형태로 포함
- 일본어는 자연스러운 일본어로 번역 (영어 단어 혼용 금지: "Cops" → "警察", "Robbers" → "泥棒")
- 게임 타이틀 `경찰과도둑`은 일본어에서도 `Cops and Robbers` 브랜드명 유지
...
1. チュートリアルガイド追加
- 待機室とゲーム画面に初めて入った際、機能案内のガイドが表示されます。

2. クレジットページ追加
- 設定 > アプリバージョンを5回タップすると、隠しクレジットページに入れます。

3. 再接続後の泥棒の足跡復元
- 接続が切れても再接続すると、以前の泥棒の位置記録が維持されます。

4. 安定性の改善とバグ修正
- ニックネーム変更後にローディングが終了しない問題を修正しました。
- ルーム参加時に自動的に待機室画面へ移動します。
