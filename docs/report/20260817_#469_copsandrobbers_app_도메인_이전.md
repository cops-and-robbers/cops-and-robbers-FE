...
**보고서 파일**: `docs/report/20260817_#469_copsandrobbers_app_도메인_이전.md`
...
- **파일**: `docs/DEEPLINK.md`
- **변경 내용**: 구 도메인 참조 19곳을 신 도메인으로 교체. `.app` 이 HSTS 프리로드 TLD 라 HTTP 접속 자체가 차단된다는 설명 추가
- **이유**: 팀 딥링크 기준 문서라 구 도메인이 남아 있으면 다음에 설정하는 사람이 만료될 도메인으로 맞춘다. `.app` 은 인증서가 없으면 도메인이 아예 열리지 않아 검증 파일 확인도 못 하므로, 원인을 못 찾고 헤매지 않도록 명시했다
