...
abstract interface class GameEntryGate {
  /// 위치 권한을 평가한다.
  ///
  /// 통과하면 true. 차단되면 안내 다이얼로그를 띄우고 **닫힐 때까지
...
/// 실제 권한 판정을 수행하는 구현체.
///
/// home_page 의 기존 `_ensureLocationPermission` 로직을 그대로 이전했다.
/// `State.mounted` 는 `context.mounted` 로 치환.
class GameEntryGateImpl implements GameEntryGate {
  ...
  Future<bool> ensure({
    ...
    return _ensureLocation(context, locationContext);
  }
...
GameEntryGate gameEntryGate(Ref ref) => GameEntryGateImpl();
